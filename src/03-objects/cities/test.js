...
class Community {
  ...
  getMostNorthern() {
    const allLatitude = this.allCity
      .map(each => each.latitude)
      .filter(each => each !== undefined);
    const mostNorthernLatitude = Math.max(...allLatitude);
    const mostNorthernCities = this.allCity.filter(each =>
      each.latitude === mostNorthernLatitude ? each : undefined
    );
  ...
  getMostSouthern() {
    ...
    const mostSouthernCities = this.allCity.filter(each =>
      each.latitude === mostSouthernLatitude ? each : undefined
    );
    return mostSouthernCities;
  }

  getPopulation() {
    const allPop = this.allCity
  ...
  createCity(newCity) {
    ...
    console.log(createCtity());
  }

  deleteCity(obj) {
    const newCityArr = this.allCity.filter(
      eachCity => eachCity.name != obj.name
    );
