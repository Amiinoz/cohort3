import { Account, AccountController } from "./account.js.js";
...
describe("Account Controller Testing", () => {
  ...
  test("getAccounts() returns array of all accounts", () => {
    ...
    expect(banker.getAccounts()).toEqual([
      { currentBalance: 5, name: "Savings Account" },
      { currentBalance: 75, name: "Checking Account" }
    ]);
  });

  test("getAccount(name) returns account by name", () => {
    expect(banker.getAccount("Checking Account")).toEqual({
      currentBalance: 75,
  ...
  test("mostValuableAccount() returns account with highest balance", () => {
    expect(banker.mostValuableAccount()).toEqual({
      currentBalance: 500,
      name: "Joint Account"
    });
  });
  ...
  test("leastValuableAccount() returns account with highest balance", () => {
    expect(banker.leastValuableAccount()).toEqual({
      currentBalance: 25,
